```
TIMEBOX:    2-3 days max. We mean it! Set at timer and hard-stop at 3 days ⏱
LANGUAGES:  PHP, Javascript, Python, SQL
FRAMEWORKS: Laravel, NodeJS, Django, etc (Exercise B can be pure SQL)
...
> :rotating_light: :exclamation: :point_right: **Please use one of the salary datasets provided here for either exercise [/datasets](/datasets)** :point_left: :exclamation: :rotating_light:
...
* Create a schema for storing the compensation data provided in one of the [available data sets](/datasets). This schema should be in at least [3NF](https://en.wikipedia.org/wiki/Third_normal_form) with tables for **employee**, **role**, and anything else that makes sense for the data given.
